 ...
 *   node scripts/plan-canvas.cjs await <file> --reply "Updated section 3."
 *   node scripts/plan-canvas.cjs end <file>
...
const {
  ...
  resolveStateDir,
  sessionKeyFor
} = require('./lib/plan-canvas/sessions.cjs');
const {
  DEFAULT_HOST,
  createPlanCanvasServer,
  resolveIdleTimeoutMs,
  resolvePort
} = require('./lib/plan-canvas/server.cjs');
...
function usage() {
  return [
    ...
    '',
    'Usage:',
    '  node scripts/plan-canvas.cjs                      Show server status and sessions',
    '  node scripts/plan-canvas.cjs open <file>          Open (or resume) a review session',
    '  node scripts/plan-canvas.cjs await <file>         Block until the human sends feedback',
    '  node scripts/plan-canvas.cjs end <file>           End a session as the agent',
    '  node scripts/plan-canvas.cjs stop                 Shut down the canvas server',
    '  node scripts/plan-canvas.cjs server               Run the server in the foreground',
    '',
    'Options:',
    ...
    '',
    'Environment: HKX_PLAN_CANVAS_PORT, HKX_PLAN_CANVAS_STATE_DIR, HKX_PLAN_CANVAS_IDLE_MS'
  ].join('\n');
}
...
async function healthCheck(port) {
  try {
    ...
    return res.body && res.body.app === 'hkx-plan-canvas' ? res.body : null;
  } catch {
    return null;
  }
}
...
async function ensureServer({ stateDir, port }) {
  ...
  const child = spawn(process.execPath, [__filename, 'server', '--port', String(port)], {
    ...
    env: { ...process.env, HKX_PLAN_CANVAS_STATE_DIR: stateDir }
  });
  child.unref();
  fs.closeSync(logFd);
  for (let i = 0; i < 50; i++) {
...
async function cmdOpen(file, args, { stateDir, port }) {
  ...
  return {
    ...
    next_step:
      'Run `hkx-plan-canvas await <file>` and leave it running; it returns when the human sends feedback, a verdict, or ends the session.'
  };
}
...
async function cmdAwait(file, args, { stateDir, port }) {
  ...
  if (result.status === 'feedback') {
    result.next_step = result.sessionEnded
      ...
      : 'Address the feedback, then run `hkx-plan-canvas await <file> --reply "<what you changed>"` to answer in the canvas and keep listening.';
