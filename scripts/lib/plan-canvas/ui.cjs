 ...
 *
 * Visual language mirrors the HKX plan canvas (scripts/dashboard-web.js):
...
const { escapeHtml } = require('./markdown.cjs');
...
// air-gapped mirror) via HKX_PLAN_CANVAS_MERMAID_URL. If the fetch fails, the
...
function mermaidUrl(env = process.env) {
  const override = env.HKX_PLAN_CANVAS_MERMAID_URL;
...
// the HKX canvas. Kept import-only so a CDN failure degrades gracefully.
...
(() => {
  ...
  const QKEY = 'hkx-plan-canvas:queue:' + key;
  let queue = [];
  ...
  // --- theme ---------------------------------------------------------
  const themeKey = 'hkx-plan-canvas:theme';
