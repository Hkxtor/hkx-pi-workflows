...
const { buildAllowedHostnames, isAllowedHostHeader, isAllowedOrigin } = require('../loopback-guard.cjs');
const { renderMarkdown } = require('./markdown.cjs');
const { artifactSdkJs } = require('./sdk.cjs');
const {
  canvasCss,
  canvasClientJs,
  renderCanvasHtml,
...
function resolvePort(env = process.env) {
  const value = Number.parseInt(env.HKX_PLAN_CANVAS_PORT || '', 10);
  return Number.isInteger(value) && value >= 0 && value <= 65535 ? value : DEFAULT_PORT;
}

function resolveIdleTimeoutMs(env = process.env) {
  const raw = String(env.HKX_PLAN_CANVAS_IDLE_MS || '').trim().toLowerCase();
    ...
    Promise.resolve()
      .then(() => {
        if (req.method === 'GET' && pathname === '/health') {
          return sendJson(res, 200, { ok: true, app: 'hkx-plan-canvas', version });
        }
