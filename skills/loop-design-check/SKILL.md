...
- A plain timer / poll → no design needed.
- *How to wire the loop architecture* (pipelines → DAGs, long-run recovery) → that's the mechanism layer; see `parallel-execution-optimizer` / `agent-introspection-debugging`. **This skill only covers "is the goal right, and will it run away" — it does not re-explain mechanism.**
...
|---|---|---|
| Has a clear "done" test (write to done / a batch of images processed) | **servo** (closed-loop) | stops on reaching the goal |
| No endpoint, must keep maintaining a state (inventory alert / scheduled health check) | **regulator** (thermostat) | never stops; acts only on change (dead-band suppresses noise) |
...
| Must "ensure something happens on time" | wrap the above in a schedule | cron fires it |
...
| 5 | Bloated `AGENTS.md`/context + stale memory → **the faster it loops, the more it errs** | Are the docs/memory it depends on fresh? Who maintains them? | Layered memory + periodic lint |
