...
## Pairing with Session-End Verification

If your workflow includes a session-end check that asks "what did you learn this session," this skill is *what to write* so that check actually has useful content rather than an empty timestamp. Enforcement without methodology → empty entries. Methodology without enforcement → forgotten captures. Each is independently useful; together they close the loop.

```
Task completes → session-end check: was the learning file touched today?
