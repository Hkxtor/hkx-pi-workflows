...
Does **not** touch personal/inherited instincts. Use `/hkx-instinct-decay` to lower confidence on those.
...
3. After apply, re-run `/hkx-instinct-status` (with pending) and `/hkx-instinct-projects`
4. Prefer `/hkx-instinct-accept` for still-useful pending items instead of letting them expire
