...
1. Discover rules the same way as `/hkx-hookify-list` (project `.pi/` + global `~/.pi/agent/hookify/`).
...
- Never rewrite message body or pattern unless user explicitly requests an edit (prefer `/hkx-hookify` for new rules).
