...
| --- | --- |
| `/prp-commit` | Stages all, auto message |
| `/prp-commit staged` | Commits only the index |
| `/prp-commit *.ts` | Stages TypeScript files |
| `/prp-commit except tests` | Stages non-test changes |
| `/prp-commit the database migration` | Stages migration-related paths |
| `/prp-commit only new files` | Stages untracked only |

## Safety

- Never force-add ignored secrets (`.env`, key files); warn and skip
- Never amend unless the user explicitly asks
- Never push in this command (use `/prp-pr` or `git push`)
