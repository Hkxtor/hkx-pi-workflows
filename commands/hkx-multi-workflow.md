...
# /hkx-multi-workflow — Multi-Workflow Router
...
2. Surfaces: slash router only; delegates to existing `hkx-workflow`, orch skills, and chains — no new runtime code.
...
Default path — single-session orchestrator (same 6 phases as `hkx-workflow`):

1. Follow **`/hkx-workflow`** phases: Research → Ideation → Plan → Execute → Verify → Review for **task**.
2. When the task is clearly a **net-new feature** and the user wants multi-agent explore/plan/TDD/review, prefer:
   - Skill/command: **`hkx-orch-add-feature`** (human gates), **or**
   - Chain: **`hkx-feature-flow`** via pi-subagents (`explore → plan → tdd → parallel review`).
3. After implementation, optional stronger review: chain **`hkx-adversarial-review`** or command **`/hkx-orch-review`** / **`/hkx-santa-loop`**.

Do **not** re-implement the six phases inline if you can invoke `/hkx-workflow` or the chain/skill above.
...
- Use **`/hkx-plan`** (or **`/hkx-blueprint`** for multi-session construction, **`/hkx-plan-prd`** for PRD-shaped work).
...
- Prefer by intent:
  - **`hkx-orch-add-feature`** / **`hkx-orch-change-feature`** / **`hkx-orch-fix-defect`** / **`hkx-orch-refine-code`**, **or**
...
- Finish with verify + **`/hkx-orch-review`** or chain **`hkx-pr-review`**.
...
- Review: language chain if known (`hkx-go-review`, `hkx-rust-review`, `hkx-python-review`; TS via `hkx-pr-review` / typescript reviewer agents), else **`hkx-pr-review`** + **`hkx-security-scan`** when auth/data touched.
...
| --- | --- |
| **review** | `/hkx-orch-review` or chains `hkx-pr-review` / `hkx-adversarial-review` (+ language `hkx-*-review` when dominant language is clear) |
| **fix** | `/hkx-orch-fix-defect` or chain `hkx-fix-defect` |
| **refactor** | `/hkx-orch-refine-code` or chain `hkx-refactor-flow` |
| **docs** | `/hkx-update-docs` or chain `hkx-docs-update` |
| **build-fix** | `/hkx-build-fix` or chains `hkx-build-fix` / `hkx-typescript-build-fix` / `hkx-go-build-fix` / `hkx-rust-build-fix` / `hkx-python-build-fix` |
| **security** | `/hkx-security-scan` or chain `hkx-security-scan` |
...
- Validation (if any)
- Next step (e.g. `execute` after `plan`, or `/hkx-prp-pr`)
...
| You say | Routes to |
| --- | --- |
| `/hkx-multi-workflow add rate limiting to the API` | **full** → `/hkx-workflow` or `hkx-feature-flow` / orch-add-feature |
| `/hkx-multi-workflow plan migrate sessions to redis` | **plan** → `/hkx-plan` (no code) |
| `/hkx-multi-workflow execute` + approved plan | **execute** → orch / feature-flow |
| `/hkx-multi-workflow backend harden auth cookies` | **backend** → feature/security-aware path |
| `/hkx-multi-workflow frontend polish settings form a11y` | **frontend** → feature-flow + frontend skills |
| `/hkx-multi-workflow review` | **review** → orch-review / pr-review chain |
| `/hkx-multi-workflow fix flaky payment webhook test` | **fix** → orch-fix-defect / fix-defect chain |

## Related

- `/hkx-workflow` — in-session 6-phase without mandatory multi-agent fan-out
- `/hkx-recipes` — browse/match command families (advisory)
