...
- Windows-only defaults: on a `win32` install, `mergeAgentSettings` seeds PowerShell 7 (`shellPath`) and a pi-native `defaultTools` list **only if missing** — these machine-local defaults are generated by the OS-aware install path, never versioned in `agent-settings.json`
