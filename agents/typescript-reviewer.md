...
4. Run ESLint if available (e.g. `npx eslint .` or equivalent packages/run scripts) — if linting or TypeScript checking fails, stop and report.
...
6. Focus on modified files and read surrounding context using `read` or `ffgrep` before commenting.
7. Begin review.
...
For detailed TypeScript and JavaScript patterns, use skill `typescript-workflow` and rule `hkx-typescript` along with `coding-standards`, `frontend-patterns`, or `backend-patterns` based on the code being reviewed.
